...
static void deh_procPointer(DEHFILE *fpin, FILE* fpout, char *line) { // done
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
        ...
        } else if (fpout) fprintf(fpout,"Invalid frame pointer index for '%s' at %lld\n",
                                      key, value);
    }
    return;
}
...
static void deh_procAmmo(DEHFILE *fpin, FILE* fpout, char *line) {
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
        ...
        else if (fpout) fprintf(fpout,"Invalid ammo string index for '%s'\n",key);
    }
    return;
}
...
static void deh_procWeapon(DEHFILE *fpin, FILE* fpout, char *line) {
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
        ...
        else if (fpout) fprintf(fpout,"Invalid weapon string index for '%s'\n",key);
    }
    return;
}
